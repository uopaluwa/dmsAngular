...
UserController.prototype.createUser = function(req, res) {
  ...
  User.findOne({
    ...
    if (err) {
      ...
      return res.json({ success: false, message: 'user email taken'});
    } else {
      User.create(req.body, function(err, user) {
        if (err) {
          return res.json(err);
        }
        return res.json({message:'user account created' , user: user});
      });
    }
  });
};
...
UserController.prototype.getCurrentUser = function(req, res) {
  var id = req.headers['u_id'] || req.body.u_id || req.params.u_id;
